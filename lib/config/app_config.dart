...
class AppConfig {
  ...
  // Redirects
  static String get webRedirectUrl => const String.fromEnvironment(
            'WEB_REDIRECT_URL',
            defaultValue: '',
          ) !=
          ''
  ...
  // Validation
  //
  // `.env.example` is a committed placeholder file that the app falls back to
  // when no real configuration is present, so "non-empty" is not enough — its
  // placeholder values would sail through and produce an app that boots and
  // then 401s on every request. Reject the placeholders explicitly.
  ...
  static bool _isReal(String value) =>
      value.isNotEmpty && !_placeholders.contains(value);

  static bool get isConfigured =>
      _isReal(supabaseUrl) && _isReal(supabaseAnonKey);
}
