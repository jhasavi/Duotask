-- Baseline schema for DuoTask.
--
-- This is supabase/schema.sql promoted into the migration chain. It was
-- previously applied by hand in the Supabase SQL Editor and existed nowhere in
-- migrations/, which is why `supabase migration list` showed the local chain
-- and the live database had diverged: the on_auth_user_created trigger (which
-- auto-creates the public.users profile row) lived only in schema.sql.
--
-- With this file in place, `supabase db push` provisions an empty project into
-- the same shape as production, which is what makes an automated test project
-- — and therefore automated integration testing — possible.
--
-- Statements are guarded (IF NOT EXISTS / OR REPLACE / DROP ... IF EXISTS) so
-- applying this to an already-provisioned database is a no-op.
