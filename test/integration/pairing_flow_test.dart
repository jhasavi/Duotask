...
void main() {
  ...
  Future<String> createUser(String label) async {
    ...
    await admin.from('users').upsert({
      ...
      'pairing_code': 'T${stamp.toString().substring(stamp.toString().length - 7)}',
    }, onConflict: 'id',);
  ...
  test('two users can pair via a code and both see the active pairing',
      () async {
    ...
    expect(serviceA.isPaired, isTrue, reason: 'requester does not see pairing');
    expect(serviceB.isPaired, isTrue, reason: 'recipient does not see pairing');
  ...
  }, tags: ['integration'], skip: skip,);

  test('an already-used pairing code cannot be redeemed twice', () async {
    final userA = await createUser('a');
    final userB = await createUser('b');
    ...
    expect(await PairingService(admin).acceptPairingCode(userB, code!), isTrue);
  ...
  test('unpairing clears the partner for both users', () async {
    final userA = await createUser('a');
    final userB = await createUser('b');
    ...
    expect(await serviceA.unpair(userA), isTrue,
  ...
  test('a paired task created by one user is visible to the other', () async {
    final userA = await createUser('a');
    final userB = await createUser('b');
